...
function createPlayer(scene, x, z) {
    ...
    scene.registerBeforeRender(() => {
        ...
        if (window.keys.left)     move.x -= 1;
        if (window.keys.right)    move.x += 1;
...
function receive(data) {
    ...
    if (cmd === "move" && parts.length === 3) {
        ...
        if (action in window.keys) window.keys[action] = state;
    }
        ...
        if (!active) {
            window.keys.forward = window.keys.backward = window.keys.left = window.keys.right = false;
        }
    }
