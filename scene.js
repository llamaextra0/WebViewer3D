...
const wallTexture = new BABYLON.Texture("./textures/brick", scene);

wallMaterial.diffuseTexture = wallTexture;
wallMaterial.diffuseTexture.uScale = 3.0; // Repeat texture horizontally
wallMaterial.diffuseTexture.vScale = 3.0; // Repeat texture vertically
...
function createPlayer(scene, x, z) {
    ...
    // FIXED COLLISION — perfect for 1x1 cells
    cam.ellipsoid = new BABYLON.Vector3(0.35, 0.9, 0.35);  // Tight fit, no clipping!
    cam.checkCollisions = true;
    cam.applyGravity = false;
    cam.minZ = 0.05;  // Prevent near-plane clipping
    cam.keysUp = cam.keysDown = cam.keysLeft = cam.keysRight = [];

    const canvas = scene.getEngine().getRenderingCanvas();

    // Look controls (unchanged)
    let dragging = false;
    let prevX, prevY;
