async function createScene(scene) {
    ...
    window.keys = { forward: false, backward: false, left: false, right: false};
    window.motion = "idle";
    window.keyboard = { forward: false, backward: false, left: false, right: false };
    // Floor
...
function createPlayer(scene, x, z) {
    ...
    document.addEventListener("keydown", (e) => {
        if (!window.gameActive) return;
        switch (e.code) {
            case "KeyW": case "ArrowUp":    window.keyboard.forward  = true; break;
            case "KeyS": case "ArrowDown":  window.keyboard.backward = true; break;
            case "KeyA": case "ArrowLeft":  window.keyboard.left     = true; break;
            case "KeyD": case "ArrowRight": window.keyboard.right    = true; break;
        }
    });
    document.addEventListener("keyup", (e) => {
        switch (e.code) {
            case "KeyW": case "ArrowUp":    window.keyboard.forward  = false; break;
            case "KeyS": case "ArrowDown":  window.keyboard.backward = false; break;
            case "KeyA": case "ArrowLeft":  window.keyboard.left     = false; break;
            case "KeyD": case "ArrowRight": window.keyboard.right    = false; break;
        }
    });
    scene.registerBeforeRender(() => {
        ...
        if (window.keys.forward||window.keyboard.forward)  move.z -= 1;
        if (window.keys.backward||window.keyboard.backward) move.z += 1;
        if (window.motion === "left" || window.keys.left||window.keyboard.left)     move.x -= 1;
        if (window.motion === "right" || window.keys.right||window.keyboard.right)   move.x += 1;
...
function receive(data) {
    ...
    if (cmd === "move" && parts.length === 3) {
        ...
        if (action in window.keys) {
            window.keys[action] = state;
        }
    }
    else if (cmd === "motion" && parts.length === 2) {  
        window.motion = parts[1];  // "left", "right", "idle"
    }
