async function createScene(scene) {
    ...
    window.keys = { forward: false, backward: false, left: false, right: false,motion: "idle" };
    // Floor
...
function createPlayer(scene, x, z) {
    ...
    scene.registerBeforeRender(() => {
        ...
        if (window.keys.forward)  move.z -= 1;
        if (window.keys.backward) move.z += 1;
        if (window.keys.motion === "left" || window.keys.left)     move.x -= 1;
        if (window.keys.motion === "right" || window.keys.right)   move.x += 1;
...
function receive(data) {
    ...
    if (cmd === "move" && parts.length === 3) {
        ...
        window.keys.motion = parts[1];  // "left", "right", "idle"
    }
